...
# For the Stop hook: decide whether to recommend cleanup at session end
#
...
if [ -f "Plans.md" ]; then
  ...
  if [ "$COMPLETED_TASKS" -ge 10 ]; then
    ...
    MESSAGE="Cleanup recommended: ${COMPLETED_TASKS} completed task(s) (say \"clean up\" to launch the maintenance skill)"
  fi
