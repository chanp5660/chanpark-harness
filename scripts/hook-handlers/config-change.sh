...
#
# Fires when a config file changes. Records to the timeline only while breezing is active.
...
# Return ok immediately if jq is unavailable
...
# Read the hook payload from stdin (with size limit and timeout)
PAYLOAD=""
if [ ! -t 0 ]; then
  if [ -n "$_TIMEOUT" ]; then
    PAYLOAD=$($_TIMEOUT 5 head -c 65536 2>/dev/null || true)
  else
    # No timeout available: use dd to enforce the byte-count cap (POSIX standard)
    PAYLOAD=$(dd bs=65536 count=1 2>/dev/null || true)
  fi
fi

# Record to the timeline only while breezing is active
if [ "$BREEZING_ACTIVE" = true ] && [ -n "$PAYLOAD" ]; then
  STATE_DIR="${PROJECT_ROOT}/.claude/state"
  mkdir -p "$STATE_DIR" 2>/dev/null || true

  # Normalize file_path to a repo-relative path (hide username, etc.)
