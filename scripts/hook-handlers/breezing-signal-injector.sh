...
# On the UserPromptSubmit hook, read unconsumed signals from breezing-signals.jsonl
# and inject them as a systemMessage.
#
# Usage: invoked automatically (UserPromptSubmit hook)
...
# Treat lines where consumed_at is null or missing as unconsumed
UNCONSUMED_SIGNALS=""
...
while IFS= read -r signal_line; do
  ...
  case "${signal_type}" in
    ci_failure_detected)
      ...
      SYSTEM_MESSAGE="${SYSTEM_MESSAGE}[SIGNAL:ci_failure_detected] CI failed (${conclusion}). Trigger: ${trigger_cmd}. Consider auto-fixing with the ci-cd-fixer agent.\n"
      ;;
    retake_requested)
      reason=""
      task_id=""
      ...
      SYSTEM_MESSAGE="${SYSTEM_MESSAGE}[SIGNAL:retake_requested] A redo was requested for task #${task_id}. Reason: ${reason}\n"
      ;;
    reviewer_approved)
      task_id=""
      if command -v jq >/dev/null 2>&1; then
    ...
    escalation_required)
      ...
      SYSTEM_MESSAGE="${SYSTEM_MESSAGE}[SIGNAL:escalation_required] Escalation is required for task #${task_id}. Reason: ${reason}\n"
      ;;
    *)
      # Notify unknown signals as-is
...
# === Set consumed_at to mark signals as consumed ===
# Atomic update: write a new file with consumed_at added, then overwrite
...
if [ "${_lock_acquired}" -eq 1 ]; then
  ...
  if command -v jq >/dev/null 2>&1; then
    # Add consumed_at and rewrite all signals
    while IFS= read -r line; do
      [ -z "${line}" ] && continue
      consumed="$(printf '%s' "${line}" | jq -r '.consumed_at // "null"' 2>/dev/null)"
      if [ "${consumed}" = "null" ]; then
...
if command -v jq >/dev/null 2>&1; then
  ...
  # Simple escaping when jq is unavailable
