...
# On PostToolUse (Bash matcher), asynchronously check CI status after git push / gh pr.
# When a CI failure is detected, inject an additionalContext message recommending a ci-cd-fixer spawn.
#
...
# === Get the command and exit code from the Bash tool output ===
TOOL_NAME=""
BASH_CMD=""
BASH_EXIT_CODE=""
BASH_OUTPUT=""
...
# === Determine whether this is a git push / gh pr command ===
...
# Poll the CI check for up to 60 seconds (only when the gh command exists)
...
check_ci_status_async() {
  ...
  while [ "${elapsed}" -lt "${max_wait}" ]; do
    ...
    # Anything other than completed is still running
    ...
    # On CI failure, write to the signals file
...
# Run the CI check in the background (do not block the hook)
check_ci_status_async &
disown 2>/dev/null || true

# === Check for a recent CI failure signal and inject additionalContext ===
ADDITIONAL_CONTEXT=""
SIGNALS_FILE="${STATE_DIR}/breezing-signals.jsonl"
