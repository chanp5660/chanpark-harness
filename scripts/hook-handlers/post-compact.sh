...
# If WIP tasks exist, restore the state saved by PreCompact and inject it as systemMessage.
# If a structured handoff artifact exists, prefer it and re-inject the high-signal key points.
#
...
# Extract WIP tasks from Plans.md and build a summary
get_wip_summary() {
...
" "${PLANS_FILE}" 2>/dev/null)" || wip_lines=""
    ...
    # Fall back to grep when python3 is unavailable
...
# Get the WIP task summary
WIP_SUMMARY="$(get_wip_summary)"

# Prefer restoring from the structured handoff artifact
...
# Thin snapshot restore for compatibility
...
# Build systemMessage
# Restore the WIP info saved by PreCompact so the model still understands task state after compaction
SYSTEM_MESSAGE=""
