...
# On the UserPromptSubmit hook, notify about pending fix proposals and,
# when an approve/reject prompt is received, apply them to Plans.md.
...
if path_has_symlink_component_within_root "${STATE_DIR}" "${PROJECT_ROOT}" || [ -L "${PENDING_FIX_PROPOSALS_FILE}" ]; then
  emit_system_message "⚠️ Aborted because the fix proposal state path is a symlink."
  exit 0
fi

if [ -e "${PLANS_PATH}" ] && path_has_symlink_component_within_root "${PLANS_PATH}" "${PROJECT_ROOT}"; then
  emit_system_message "⚠️ Cannot apply the fix proposal because the Plans.md path is a symlink."
  exit 0
fi
...
if [ -n "${ACTION}" ] && [ -z "${TARGET_ID}" ] && [ "${PENDING_COUNT}" != "1" ]; then
  emit_system_message "⚠️ There are ${PENDING_COUNT} unprocessed fix proposals. Use approve fix <task_id> or reject fix <task_id> to specify the target."
  exit 0
fi
...
if [ "${ACTION}" = "approve" ]; then
  ...
  case "${APPLY_RESULT}" in
    applied|already_present)
      ...
      emit_system_message "✅ Applied the fix proposal: ${FIX_TASK_ID}\nDetails: ${PROPOSAL_SUBJECT}"
      exit 0
      ;;
    plans_missing)
      emit_system_message "⚠️ Could not apply the fix proposal. Plans.md was not found."
      exit 0
      ;;
    *)
      emit_system_message "⚠️ Failed to apply the fix proposal. Target task ${SOURCE_TASK_ID} was not found in Plans.md."
      exit 0
      ;;
  esac
fi
...
if [ "${ACTION}" = "reject" ]; then
  consume_proposal "${SOURCE_TASK_ID}" >/dev/null 2>&1 || true
  emit_system_message "ℹ️ Rejected the fix proposal: ${FIX_TASK_ID}"
  exit 0
fi

REMINDER="[FIX PROPOSAL] There are unprocessed fix task proposals (${PENDING_COUNT})\n"
