...
#
# Fires when stopping a session fails due to an API error (rate limit, auth failure, etc.).
# Logs the error info and, in Breezing mode, attempts to notify the Lead.
#
...
# Call detect_project_root only after confirming it is defined
...
# State directory (scoped per project when CLAUDE_PLUGIN_DATA is used)
...
# === Log to JSONL (safely escaped with jq/python3) ===
log_entry=""
if command -v jq > /dev/null 2>&1; then
  log_entry="$(jq -nc \
...
# === Notify the Lead (on 429 rate limit) ===
if [ "${ERROR_CODE}" = "429" ]; then
  # Notify the Lead via systemMessage (CC hooks protocol)
  # Build JSON with SESSION_ID safely escaped via jq/python3
  _sys_msg=""
  if command -v jq > /dev/null 2>&1; then
    _sys_msg="$(jq -nc --arg sid "${SESSION_ID}" \
      '{systemMessage: ("[StopFailure] Worker " + $sid + " stopped due to rate limit (429). The Breezing Lead should attempt automatic resume after exponential backoff.")}')"
  elif command -v python3 > /dev/null 2>&1; then
    _sys_msg="$(python3 -c "
import json, sys
print(json.dumps({'systemMessage': '[StopFailure] Worker ' + sys.argv[1] + ' stopped due to rate limit (429). The Breezing Lead should attempt automatic resume after exponential backoff.'}, ensure_ascii=False))
