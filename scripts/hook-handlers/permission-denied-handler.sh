...
# Records the denial event to telemetry, and includes a notification to the Lead in Breezing mode.
# Returning {retry: true} tells the model that a retry is possible.
#
...
# === For a Breezing Worker: notify the Lead + retry hint ===
# When a Worker is denied, inform the Lead so it can consider alternatives
# Returning retry: true tells the model that a retry is possible
_notification_text="[PermissionDenied] The Worker's tool ${TOOL_NAME} was denied in auto mode. Reason: ${DENIED_REASON}. Consider an alternative approach, or approve it manually if needed."
_is_worker=false
if [ "${AGENT_TYPE}" = "worker" ] || [ "${AGENT_TYPE}" = "task-worker" ] || echo "${AGENT_TYPE}" | grep -qE ':worker$'; then
  _is_worker=true

  # Write to the broadcast file (so the Lead session can read it)
...
# For non-Workers, pass through (no retry — leave the decision to the user)
