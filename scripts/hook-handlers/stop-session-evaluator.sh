...
#
# A command-type hook that reliably outputs valid JSON, as a replacement for the prompt type.
# Inspects session state and decides whether to allow or block stopping.
# CC 2.1.47+: read last_assistant_message from stdin and record it in session.json.
#
...
# Return ok immediately if jq is unavailable (safe fallback)
...
# Read the hook payload from stdin (with size limit + timeout)
PAYLOAD=""
if [ -t 0 ]; then
  ...
  if [ -n "$_TIMEOUT" ]; then
    ...
    # No timeout available: enforce byte limit with dd (POSIX standard)
...
# Return ok immediately if the state file is absent
...
# Return ok immediately if stopping is already processed
...
# When the user explicitly pressed Stop, allow stopping by default
echo '{"ok":true}'
exit 0
