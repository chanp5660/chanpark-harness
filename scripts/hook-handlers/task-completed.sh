...
# Records to the timeline when a task completes
#
...
# JSONL rotation (truncate to 400 lines when over 500 lines)
rotate_jsonl() {
  local file="$1"
  local _lines
...
# === Timeline logging (build JSON safely with jq -nc) ===
ensure_state_dir
TS="$(get_timestamp)"
...
if [ -f "${BREEZING_ACTIVE}" ]; then
  # Get the session ID (for session-scoping the signals)
  SESSION_ID=""
  if command -v jq >/dev/null 2>&1; then
...
" "${BREEZING_ACTIVE}" 2>/dev/null)" || SESSION_ID=""
  ...
  # Get the current batch's task ID list and total task count from breezing-active.json
  TOTAL_TASKS=0
  CURRENT_BATCH_IDS=""
  if command -v jq >/dev/null 2>&1; then
    # Progressive Batching: read from the current in_progress batch
    _batch_info="$(jq -r '
    ...
    # When not batching, estimate from the number of plans_md_mapping keys
  ...
  # Count completions only within the current batch (exclude prior batch completions)
  # +1 per task ID if present (prevents duplicate counting from retakes, etc.)
  ...
  if [ -f "${TIMELINE_FILE}" ] && [ -n "${CURRENT_BATCH_IDS}" ]; then
    ...
    # Fallback: count overall when batch IDs cannot be obtained
  ...
  # If SESSION_ID is set, dedup per session (AND within the same record); otherwise dedup globally
  _signal_exists() {
    local sig_type="$1"
    if [ -n "${SESSION_ID}" ] && [ -f "${SIGNALS_FILE}" ]; then
      # Whether a record on the same line contains both signal and session_id
...
" "${sig_type}" "${SESSION_ID}" "${completed}" "${total}" "${ts}" 2>/dev/null
  ...
  # -ge handles overshooting the threshold (e.g. simultaneous completions); already-emitted check prevents duplicates
  # Ceiling arithmetic prevents the threshold from triggering too early
  # HALF>1 guard: partial review is unnecessary for batch sizes 1-2 (full review suffices)
  if [ "${TOTAL_TASKS}" -gt 0 ] 2>/dev/null; then
    ...
    # 60% completion signal: recommend registering the next batch (for Progressive Batch)
...
# === Test result lookup logic ===
# Check the auto-test-runner result file; escalate with exit 2 on not-run/failure
TEST_RESULT_FILE="${STATE_DIR}/test-result.json"
QUALITY_GATE_FILE="${STATE_DIR}/task-quality-gate.json"

# Update the per-task-ID failure count
...
update_failure_count() {
  ...
  if command -v jq >/dev/null 2>&1; then
    ...
    # Update the file (create it if it does not exist)
    local existing="{}"
    if [ -f "${QUALITY_GATE_FILE}" ]; then
...
check_test_result() {
  # If the result file does not exist: tests were not run
  if [ ! -f "${TEST_RESULT_FILE}" ]; then
    return 0  # Skip when not run (some projects do not require tests)
  fi
...
if ! check_test_result; then
  # Test failure: increment the failure count
  FAIL_COUNT="$(update_failure_count "${TASK_ID}" "increment")"

  # Also record to the timeline
  FAIL_ENTRY=""
  if command -v jq >/dev/null 2>&1; then
    FAIL_ENTRY="$(jq -nc \
  ...
  if [ "${FAIL_COUNT}" -ge "${ESCALATION_THRESHOLD}" ] 2>/dev/null; then
    # Collect cause classification, recommended action, and attempt history from the test result
    _last_cmd=""
    _last_output=""
...
" "${TEST_RESULT_FILE}" 2>/dev/null)"
    ...
    # Cause classification (inferred from test output keywords)
    if echo "${_last_output}" | grep -qi "syntax\|SyntaxError\|parse error\|unexpected token"; then
      ...
      _recommended_action="A test assertion failed. Check the difference between expected and actual values."
    elif echo "${_last_output}" | grep -qi "timeout\|Timeout\|ETIMEDOUT\|timed out"; then
      _failure_category="timeout"
      _recommended_action="A timeout occurred. Check async processing and network dependencies."
    elif echo "${_last_output}" | grep -qi "permission\|EACCES\|EPERM\|access denied"; then
      _failure_category="permission_error"
      _recommended_action="A permission error occurred. Check file permissions."
    else
      _failure_category="runtime_error"
      _recommended_action="A runtime error occurred. Inspect the test output in detail."
    fi

    # Get the attempt history from the quality-gate file
    _history_summary=""
    if [ -f "${QUALITY_GATE_FILE}" ]; then
      if command -v jq >/dev/null 2>&1; then
    ...
    # Print the escalation report to stderr
    {
      echo ""
      echo "=========================================="
      echo "[ESCALATION] 3 consecutive failures detected - stopping the auto-fix loop"
      echo "=========================================="
      ...
      echo "  Manual intervention is required. The loop will end after recording the escalation."
      echo "=========================================="
      echo ""
    } >&2 2>/dev/null || true

    # Append the escalation record to the timeline
    ESC_ENTRY=""
    if command -v jq >/dev/null 2>&1; then
      ESC_ENTRY="$(jq -nc \
    ...
    _fix_dod="$(sanitize_inline_text "Resolve the failure category (${_failure_category}) and ensure the latest tests/CI pass")"
    _proposal_json=""
    if command -v jq >/dev/null 2>&1; then
...
Warning: failed to save the proposal. Add it to Plans.md manually."
    fi

    # Approve the hook with exit 0 while surfacing a proposal that can be approved next turn
    if command -v jq >/dev/null 2>&1; then
      jq -nc \
  ...
  # Test passed or not run: reset the failure count
  if [ -n "${TASK_ID}" ]; then
    update_failure_count "${TASK_ID}" "reset" >/dev/null 2>&1 || true
  fi
fi

# === Webhook notification helper (factored into a function so all exit paths call it) ===
_fire_webhook() {
...
# Response with a progress summary
_fire_webhook
