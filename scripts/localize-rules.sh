...
#
# Features:
# - Adjust paths: based on the project analysis results
...
generate_code_paths() {
  ...
  # Generate a pattern per source directory
...
generate_test_paths() {
  ...
  if [ ${#test_dirs_arr[@]} -gt 0 ]; then
    ...
    # Check the default test directories
...
# Generate from template (confirm overwrite if a file already exists)
...
- Require type definitions for Props
"
fi
...
| \`refactor:\` | Refactoring | \`refactor: tidy up auth logic\` |
...
- ❌ Refactoring that was not requested
- ❌ Excessive comments
$LANG_SPECIFIC
## Pull Request

- Title: concisely describe the change (within 50 characters)
...
1. **Boundary tests**: always test input boundary values
2. **Happy and error paths**: cover both cases
3. **Independence**: each test does not depend on other tests
4. **Clear naming**: the test name makes clear what is being tested
