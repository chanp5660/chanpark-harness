#!/bin/bash
# session-monitor.sh
# Collect and display project state at session start
#
# Usage: run automatically from the SessionStart hook
# Output: project state summary + state file generation
...
# Resolve the Plans.md path honoring the plansDirectory setting
...
# Compute a relative time (from seconds to "X minutes ago", "X hours ago", etc.)
relative_time() {
  local seconds=$1
...
if [ -f "$PLANS_FILE" ]; then
  ...
  # The English marker family is canonical. Japanese / cursor are treated as compatible synonyms
...
# If an existing session has not ended, try to resume
EXISTING_SESSION_ID=""
EXISTING_ENDED_AT=""
...
if [ "$RESUME_MODE" = "true" ] && [ -f "$STATE_FILE" ]; then
  # Update the existing session (resume)
  if command -v jq >/dev/null 2>&1; then
    tmp_file=$(mktemp)
    jq --arg cwd "$(pwd)" \
...
# Build the skills index (name + description)
SKILLS_INDEX="[]"
if [ -d "skills" ]; then
  # Collect skills as JSON (prefer jq, fall back to python)
  if command -v jq >/dev/null 2>&1; then
    SKILLS_INDEX=$(
...
if [ "$LSP_AVAILABLE" = "true" ]; then
  # When official LSP plugins are installed, map the supported extensions
...
# ================================
echo ""
echo "📊 Session start - project state"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
if [ "$CONTEXT_BUDGET_ENABLED" = "true" ]; then
  ...
  if [ "$MCP_ENABLED_ESTIMATE" != "unknown" ] && [ "$MCP_ENABLED_ESTIMATE" -gt "$CONTEXT_MAX_ENABLED_MCPS" ] 2>/dev/null; then
    echo "⚠️ Estimated enabled MCP count exceeds the limit (${MCP_ENABLED_ESTIMATE}/${CONTEXT_MAX_ENABLED_MCPS})"
  fi

  if [ "$PLUGIN_ENABLED_ESTIMATE" != "unknown" ] && [ "$PLUGIN_ENABLED_ESTIMATE" -gt "$CONTEXT_MAX_INSTALLED_PLUGINS" ] 2>/dev/null; then
    echo "⚠️ Estimated plugin count exceeds the limit (${PLUGIN_ENABLED_ESTIMATE}/${CONTEXT_MAX_INSTALLED_PLUGINS})"
  fi
fi
