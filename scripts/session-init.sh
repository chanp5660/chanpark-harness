#!/bin/bash
# session-init.sh
# SessionStart Hook: initialization at session start
#
# Features:
# 1. Check and sync plugin cache integrity
# 2. Initialize the Skills Gate
# 3. Show the Plans.md status
#
# Output: writes info to hookSpecificOutput.additionalContext in JSON
...
# ===== Banner (printed to the terminal via stderr) =====
...
# - Skip stale hook config detection
...
# Variable that accumulates the output message
OUTPUT=""
...
# Auto-repair broken symlinks caused by git clone on Windows
SYMLINK_INFO=""
...
# Clear the SSOT sync flag (at new session start)
# This flag is created when /sync-ssot-from-memory runs and is used to
# confirm SSOT sync before Plans.md cleanup
rm -f "${STATE_DIR}/.ssot-synced-this-session" 2>/dev/null || true

# Clear the work warning flag (at new session start)
...
# ===== Step 2.5: Harness session init & CC session_id mapping =====
SESSION_FILE="${STATE_DIR}/session.json"
SESSION_MAP_FILE="${STATE_DIR}/session-map.json"
ARCHIVE_DIR="${STATE_DIR}/sessions"
mkdir -p "$ARCHIVE_DIR"

# Generate a Harness session_id for a new session
NOW=$(date -u +%Y-%m-%dT%H:%M:%SZ)
HARNESS_SESSION_ID="session-$(date +%s)"

# Initialize session.json (if missing, or in a stopped state)
...
SESSEOF
  ...
  # Get the session_id of the existing session
  if command -v jq >/dev/null 2>&1; then
    HARNESS_SESSION_ID="$(jq -r '.session_id // empty' "$SESSION_FILE" 2>/dev/null)"
  fi
fi

# Save the mapping between CC session_id and Harness session_id
...
# Register self in active.json (so other sessions can detect it)
...
if [ -f "$PLANS_PATH" ]; then
  ...
  PLANS_INFO="📄 Plans.md: In progress ${wip_count} / To do ${todo_count}"
else
...
if [ -f "$TEMPLATE_TRACKER" ] && [ -f "$SCRIPT_DIR/../templates/template-registry.json" ]; then
  # Initialize if generated-files.json is missing
      ...
      if [ "$NEEDS_CHECK" = "true" ]; then
        ...
        if [ "$UPDATES_COUNT" -gt 0 ]; then
          ...
          if [ "$LOCALIZED_COUNT" -gt 0 ]; then
            parts+=("merge needed: ${LOCALIZED_COUNT}")
          fi
        fi

        # Files that need a fresh install
...
# Notify if the quality-protection rules (v2.5.30+) are not installed
MISSING_RULES_INFO=""
...
if [ -d "$RULES_DIR" ]; then
  ...
  if [ ${#MISSING_RULES[@]} -gt 0 ]; then
    MISSING_RULES_INFO="⚠️ Quality-protection rules not installed: ${MISSING_RULES[*]} → add with \`/harness-update\`"
  fi
elif [ -f ".claude-code-harness-version" ]; then
  # Harness is installed but there is no rules directory
  MISSING_RULES_INFO="⚠️ Quality-protection rules not installed → add with \`/harness-update\`"
fi

# ===== Step 6: Detect stale hook config =====
# Only detect hooks whose command path contains "claude-code-harness" (exclude user-defined hooks)
OLD_HOOKS_INFO=""
...
if [ -f "$SETTINGS_FILE" ]; then
  if command -v jq >/dev/null 2>&1; then
    ...
    if [ ${#OLD_HARNESS_EVENTS[@]} -gt 0 ]; then
      OLD_HOOKS_INFO="⚠️ Stale harness hook config detected: ${OLD_HARNESS_EVENTS[*]} → removal recommended via \`/harness-update\`"
    fi
  fi
fi
...
# SIMPLE mode warning (also emitted to additionalContext — reuses the warning text from check-simple-mode.sh)
...
if [ -n "$HANDOFF_ARTIFACT_PATH" ]; then
  # Reject stale handoffs: ignore artifacts older than 24 hours
...
add_line "|---------|------|------|"
add_line "| \`cc:todo\` | To do | Impl (Claude Code) is scheduled to run |"
add_line "| \`cc:wip\` | In progress | Impl is implementing |"
add_line "| \`cc:done\` | Done | Impl finished, awaiting approval |"
add_line "| \`cc:blocked\` | Blocked | Waiting on a dependent task |"
...
# The additionalContext content is displayed as a system-reminder
