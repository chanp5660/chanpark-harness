...
# Security: limit the bypass with an expiration (24 hours)
# Note: CWD is fetched from JSON later, so this is initialization only
...
# In --codex mode Claude is the PM, and Edit/Write are prohibited
# (implementation is delegated to the Codex Worker)
...
# (implementation is delegated to the Codex Implementer via codex exec (CLI))
...
check_work_mode() {
  ...
  # Backward compat: if work-active.json is absent, try ultrawork-active.json
  ...
  # Expiration check (within 24 hours of started_at)
  local started_at
  started_at=$(jq -r '.started_at // empty' "$active_file" 2>/dev/null)
  [ -z "$started_at" ] && return

  # ISO8601 parse (works on both macOS/Linux)
...
# Codex mode detection function (call after CWD is obtained)
# If work-active.json has codex_mode: true, block Claude's Edit/Write
# Precondition: set CODEX_MODE only when WORK_MODE is true and the TTL is valid
# Performance: prefer the value cached by check_work_mode
check_codex_mode() {
  # Skip if work mode is not active (accounts for expired TTL, etc.)
...
# Breezing-Codex mode detection function (call after CWD is obtained)
# If breezing-active.json has impl_mode: "codex", block direct Write/Edit
...
# Detection and handling of the Breezing role-registration Write
# On the teammate's first Write (breezing-role-*.json), register session_id / agent_id -> role
...
EOF
  ...
  case "$key" in
    ...
    ask_write_outside_project) echo "Confirm: writing outside the project (file_path: $arg)" ;;
    ...
    deny_git_commit_no_review) echo "Blocked: run /harness-review before committing. After review, you can run git commit again." ;;
    deny_codex_mode) echo "[Codex Mode] In --codex mode Claude is the PM. Direct Edit/Write is prohibited. Delegate implementation to the Codex Worker via codex exec (CLI)." ;;
    deny_breezing_codex_mode) echo "[Breezing-Codex] Direct Edit/Write is prohibited in codex impl mode. Implement via codex exec (CLI)." ;;
    deny_codex_mcp) echo "Blocked: Codex MCP is deprecated. Use 'codex exec' (Bash) instead. Details: .claude/rules/codex-cli-only.md" ;;
    codex_plans_symlink) echo "[Codex Mode] Symlinks are not allowed for Plans.md." ;;
    breezing_codex_symlink) echo "[Breezing-Codex] Symlinks are not allowed." ;;
    breezing_reviewer_readonly) echo "[Breezing] Reviewer is read-only. Fixing code is the Implementer's responsibility." ;;
    breezing_owns_outside) echo "[Breezing] This file is outside the owns scope: $arg" ;;
    breezing_reviewer_bash_write) echo "[Breezing] Reviewer cannot run write-like Bash commands." ;;
    breezing_reviewer_file_ops) echo "[Breezing] Reviewer cannot run file operation commands." ;;
    breezing_reviewer_git_mutation) echo "[Breezing] Reviewer cannot run git mutation commands." ;;
    breezing_implementer_git_commit) echo "[Breezing] Implementer cannot run git commit. The Lead commits in a batch at the completion stage." ;;
    breezing_implementer_git_push) echo "[Breezing] Implementer cannot run git push." ;;
    breezing_codex_file_ops) echo "[Breezing-Codex] File operation commands are prohibited in codex impl mode." ;;
    breezing_codex_git_mutation) echo "[Breezing-Codex] Git mutation commands are prohibited in codex impl mode." ;;
    codex_ask_file_ops) echo "[Codex Mode] Run file operations (mv/cp) in PM mode? Delegating implementation to the Codex Worker is recommended." ;;
    codex_ask_rm_rf) echo "[Codex Mode] Run rm -rf in PM mode? Delegating implementation to the Codex Worker is recommended. ($arg)" ;;
    codex_ask_rm) echo "[Codex Mode] Run rm in PM mode? Delegating implementation to the Codex Worker is recommended." ;;
    cost_limit_total) echo "[Cost Control] Session tool-call limit ($arg) reached. Start a new session." ;;
    cost_limit_edit) echo "[Cost Control] Edit/Write call limit ($arg) reached." ;;
    cost_limit_bash) echo "[Cost Control] Bash call limit ($arg) reached." ;;
    cost_warning_total) echo "[Cost Warning] Total tool calls: ${arg}/${arg2} (over ${arg3}%)" ;;
    cost_warning_edit) echo "[Cost Warning] Edit/Write: ${arg}/${arg2}" ;;
    cost_warning_bash) echo "[Cost Warning] Bash: ${arg}/${arg2}" ;;
    test_quality_guideline) cat <<'EOF'
...
[Implementation Quality Guideline]
- Do not hard-code test expectations
- Do not add stubs, mocks, or empty implementations
- Implement meaningful logic
EOF
      ;;
    skills_gate) cat <<EOF
...
check_cost_control() {
  ...
  if command -v jq >/dev/null 2>&1; then
    ...
    if [ -n "$warnings" ]; then
      echo -e "$warnings"
      return 2  # has warnings (not a block)
    fi
  fi
...
# The MCP server has been removed. Fail-safe in case text fixes slip through.
...
# For a warning (result=2), include it in additionalContext in later processing

# ===== additionalContext guideline generation (Claude Code v2.1.9+) =====
# On Write/Edit, return a guideline based on the file path
...
# Return: guideline string (empty if none applies)
...
# Explicitly allowing with permissionDecision: "allow" avoids the prompt
...
if [ "$TOOL_NAME" = "Write" ] || [ "$TOOL_NAME" = "Edit" ]; then
  ...
  if [ "$CODEX_MODE" = "true" ]; then
    # Allow updating Plans.md status markers (a legitimate PM operation)
    # Tighten the pattern: allow only when it ends exactly with "Plans.md"
    # Reject symlinks (security measure)
    if [ -L "$FILE_PATH" ]; then
  ...
  if { [ -n "$SESSION_ID" ] || [ -n "$AGENT_ID" ]; } && [ -n "$CWD" ]; then
    # Detect the role-registration Write (a Write to breezing-role-*.json is the registration step)
    ...
    if [ "$BREEZING_ROLE" = "implementer" ] && [ -n "$BREEZING_OWNS" ] && [ "$BREEZING_OWNS" != "null" ]; then
      ...
      case "$FILE_PATH" in
        ...
        *)
          ...
          # Compute the relative path from CWD (REL_PATH is undefined at this point)
          BREEZING_REL_PATH="$FILE_PATH"
          if [ -n "$CWD" ]; then
            BREEZING_REL_PATH="${FILE_PATH#${CWD}/}"
          fi

          # Get the owns array with jq and match
  ...
  is_excluded_path() {
    ...
    # If there is no policy file, decide based on defaults only
  ...
  # ===== LSP Gate: recommend LSP usage on semantic changes =====
  ...
  # If all guards passed, return a guideline based on the file path
...
if [ "$TOOL_NAME" = "Bash" ]; then
  ...
  if [ -n "$BREEZING_ROLE" ]; then
    ...
    if [ "$BREEZING_ROLE" = "reviewer" ]; then
      ...
      # 2>&1 (stderr->stdout) is read-safe, so exclude it
    ...
    # Implementer: block git commit (only the Lead commits)
  ...
  # ===== Breezing-Codex Mode: restrict write-like commands in Bash =====
  if [ "$BREEZING_CODEX_MODE" = "true" ]; then
    # Block redirects and in-place edits (2>&1 is read-safe, so exclude it)
  ...
  # ===== Codex Mode: PM also has write-like Bash commands restricted =====
  if [ "$CODEX_MODE" = "true" ]; then
    ...
    # Note: rm is handled later by the rm -rf whitelist, so exclude it here
    # 2>&1 (stderr->stdout) is read-safe, so exclude it
    ...
    # Ask for confirmation on mv, cp
    # rm is handled later by the rm -rf whitelist (avoids ordering issues)
  ...
  if echo "$COMMAND" | grep -Eiq '(^|[[:space:]])git[[:space:]]+commit([[:space:]]|$)'; then
    ...
    if [ "$COMMIT_GUARD_ENABLED" = "true" ]; then
      ...
      # Clear the approval state after committing (require re-review before the next commit)
      # Note: this should be done in PostToolUse, but here it is a warning only
    fi
  fi
  ...
  # Detect dangerous recursive-delete patterns of rm
  ...
  if echo "$COMMAND" | grep -Eiq '(^|[[:space:]])rm[[:space:]]+-[a-z]*r[a-z]*[[:space:]]' || \
    ...
    # ===== Work whitelist approach (Codex-approved) =====
    # Default: ask for confirmation
    RM_AUTO_APPROVE="false"

    # Check only when work mode is active and the rm_rf bypass is allowed
    if [ "$WORK_MODE" = "true" ] && [ "$WORK_BYPASS_RM_RF" = "true" ]; then

      # 0. Only the allowed flag forms (rm -rf or rm -r -f)
      # Other forms like rm -rfv, rm -fr, rm --recursive require confirmation
      if ! echo "$COMMAND" | grep -Eq '(^|[[:space:]])rm[[:space:]]+(-rf|-r[[:space:]]+-f)[[:space:]]+'; then
        : # ask for confirmation (disallowed flag form)
      # 1. Confirm if it contains dangerous shell syntax (* ? $ ( ) { } ; | & < > \ `)
      elif echo "$COMMAND" | grep -Eq '[\*\?\$\(\)\{\};|&<>\\`]'; then
        : # ask for confirmation
      # 2. Confirm if it contains sudo/xargs/find
        ...
        # 3. Single-target check (not multiple space-separated targets)
        RM_TARGET_COUNT=$(echo "$COMMAND" | sed -E 's/^.*rm[[:space:]]+(-rf|-fr|-r[[:space:]]+-f|-f[[:space:]]+-r)[[:space:]]+//' | wc -w | tr -d ' ')
        if [ "$RM_TARGET_COUNT" -eq 1 ]; then

          # 4. Relative path only (does not start with / or ~)
          # 5. No parent reference (does not contain ..)
          # 6. No trailing slash
          # 7. No path separator (basename only)
          # 8. Does not contain . or //
          case "$RM_TARGET" in
            /*|~*|*..*)
              : # ask for confirmation
              ;;
                ...
                *)
                  ...
                  if [ -n "$CWD" ]; then
                    ...
                    if [ -f "$WORK_FILE" ] && command -v jq >/dev/null 2>&1; then
                      # Get the whitelist from allowed_rm_paths
    ...
    if [ "$RM_AUTO_APPROVE" != "true" ]; then
      # In Codex mode, add the PM-oriented message
      if [ "$CODEX_MODE" = "true" ]; then
  ...
  # ===== Codex Mode: also confirm plain rm (without -r) =====
  if [ "$CODEX_MODE" = "true" ]; then
