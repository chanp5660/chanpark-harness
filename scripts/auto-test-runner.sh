#!/bin/bash
# auto-test-runner.sh - automatic test run on file change
...
# Decide whether a test run is needed
should_run_tests() {
    local file="$1"
...
run_tests() {
    ...
    # Decide the command to run (narrow to the related test if present)
    ...
    if command -v jq >/dev/null 2>&1; then
        ...
        # Emit minimal JSON if jq is unavailable
...
# Main processing
main() {
    # Check whether a test run is needed
    ...
    if [ "${HARNESS_AUTO_TEST:-}" = "run" ]; then
        ...
        # Emit the result summary to stderr (for the hooks log)
        RESULT_FILE="${STATE_DIR}/test-result.json"
        if [ -f "$RESULT_FILE" ]; then
            ...
            # Notify Claude of the test result as additionalContext
...
Fix the implementation to make the tests pass."
    ...
    # Default: recommend mode (record a test recommendation)
