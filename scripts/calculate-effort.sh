#!/bin/bash
# calculate-effort.sh
# Read task info from Plans.md, compute the effort level, and print it to stdout.
#
# Usage:
...
#   2+ dependency tasks -> +1
#   keyword (refactor, migration, security, cross-cutting) -> +1
...
# Get the task description from arguments or stdin
TASK_INPUT=""
if [ $# -gt 0 ]; then
  TASK_INPUT="$*"
elif [ ! -t 0 ]; then
...
# Resolve the Plans.md path (fall back in order: git root -> PROJECT_ROOT -> cwd)
_GIT_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || true)"
PLANS_MD="${PROJECT_ROOT:-${_GIT_ROOT:-$(pwd)}}/Plans.md"

# Extract task info from Plans.md
# Assumes v2 format: | Task | Description | DoD | Depends | Status |
TASK_CONTENT=""
TASK_DOD=""
TASK_DEPENDS=""

if [ -f "$PLANS_MD" ]; then
  # Search by task-ID pattern (#123, 34.2.2, #34.2.2 forms)
  TASK_ID_PATTERN=""
  if echo "$TASK_INPUT" | grep -qE '^#?[0-9]+(\.[0-9]+)*$'; then
    TASK_ID_PATTERN=$(echo "$TASK_INPUT" | tr -d '#')
    # Parse the table row: | number | Description | DoD | Depends | Status |
    TASK_ROW=$(grep -E "^\|[[:space:]]*${TASK_ID_PATTERN}[[:space:]]*\|" "$PLANS_MD" 2>/dev/null || true)
    if [ -n "$TASK_ROW" ]; then
      # Extract columns by pipe separator
  ...
  # If not found by task ID, keyword-search by description
  if [ -z "$TASK_CONTENT" ]; then
    # Extract the line in Plans.md closest to the task description (table row)
...
# If nothing could be obtained from Plans.md, analyze the task input itself
if [ -z "$TASK_CONTENT" ]; then
  TASK_CONTENT="$TASK_INPUT"
fi

# Text to analyze (task content + DoD + input text combined)
...
# 2. 2+ dependency tasks -> +1
if [ -n "$TASK_DEPENDS" ]; then
  # Number of dependency tasks in the Depends column (dotted ID: 34.1.1, simple ID: #123, comma-separated, etc.)
  # Count dotted IDs first, then count the remainder as simple numeric IDs
...
if [ -n "$TASK_DOD" ]; then
  # Count conditions split by semicolon / ideographic comma / comma (delimiter count + 1 = condition count)
