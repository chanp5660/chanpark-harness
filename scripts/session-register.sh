#!/bin/bash
# session-register.sh
# Register a session in active.json (no output)
#
# Usage:
...
# ===== Main =====
main() {
  # Get the session ID (argument first, otherwise from file)
  local session_id="${1:-}"
  if [ -z "$session_id" ]; then
    session_id=$(get_session_id_from_file)
  fi

  # Do nothing if there is no session ID (no error either)
  ...
  # Cleanup setup for the temp file
  local tmp_file=""
