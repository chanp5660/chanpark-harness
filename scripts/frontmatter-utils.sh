...
has_frontmatter() {
  ...
  # Whether _harness_version or _harness_template is present
  sed -n '/^---$/,/^---$/p' "$file" | grep -qE "_harness_(version|template):"
}

# Get the version from frontmatter
...
get_frontmatter_version() {
  ...
  # For JSON files, get it directly with jq (.json or .json.template)
  ...
  # Check YAML frontmatter existence
...
# Get the template name from frontmatter
...
# Get a file's version (frontmatter first, with fallback)
# Usage: get_file_version "CLAUDE.md" "generated-files.json"
get_file_version() {
  local file="$1"
  local fallback_registry="$2"

  # 1. Try to get the version from frontmatter
  local version
  ...
  # 2. Fallback: get it from generated-files.json
...
# Get the version from a YAML comment format (for .yaml files)
