...
# Check for incoming inter-session messages
#
# Usage:
...
main() {
  ...
  # If the broadcast file does not exist
  ...
  if [ "$mode" = "count" ]; then
    ...
    if [ "$unread_count" -gt 0 ]; then
      ...
      cat <<EOF
{"hookSpecificOutput":{"hookEventName":"InboxCheck","additionalContext":"📨 ${unread_count} unread message(s):\\n${escaped_messages}"}}
EOF
    else
      echo '{"hookSpecificOutput":{"hookEventName":"InboxCheck","additionalContext":""}}'
    fi
    ...
    if [ "$unread_count" -gt 0 ]; then
      ...
      echo ""
      echo "💡 Use /session inbox --mark to mark them as read"
    else
