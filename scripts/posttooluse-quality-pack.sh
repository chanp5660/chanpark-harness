...
if [ "$QUALITY_PRETTIER" = "true" ]; then
  if [ "$QUALITY_MODE" = "run" ]; then
    if run_prettier; then
      append_feedback "🧹 Prettier: ran"
    else
      append_feedback "🧹 Prettier: not run (prettier not found)"
    fi
  else
...
if [ "$QUALITY_TSC" = "true" ]; then
  if [ "$QUALITY_MODE" = "run" ]; then
    if run_tsc; then
      append_feedback "🧪 tsc --noEmit: ran"
    else
      append_feedback "🧪 tsc --noEmit: not run (tsconfig/tsc not detected)"
    fi
  else
