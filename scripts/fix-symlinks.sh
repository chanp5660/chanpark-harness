#!/bin/bash
# fix-symlinks.sh
# Detect broken symlinks / plain-text link projections in Windows environments and auto-repair them with real copies
#
# Purpose: called from session-init.sh
# Behavior:
#   - Validate the harness-* skill mirrors under codex/.codex/skills/ and opencode/skills/
#   - Repair them with real copies from skills/ (SSOT)
#   - Output the repair count to stdout (JSON format)
#
# Output:
...
# Mirror destinations (skills/ is the SSOT, so it is not checked)
MIRROR_ROOTS=(
  "codex/.codex/skills"
  "opencode/skills"
)
...
for mirror_root in "${MIRROR_ROOTS[@]}"; do
  ...
  for skill in "${HARNESS_SKILLS[@]}"; do
    ...
    # Normal: exists as a directory -> skip
    if [ -d "$mirror_path" ] && [ ! -L "$mirror_path" ]; then
      continue
    fi

    # Broken plain-text link: exists as a regular file (occurs with Windows git clone)
    ...
    # Copy if it does not exist either
