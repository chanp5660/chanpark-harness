#!/bin/bash
# diagnose-and-fix.sh
# Script that diagnoses CI errors and proposes fixes or auto-fixes them
#
# Usage:
#   ./scripts/ci/diagnose-and-fix.sh          # diagnose only
#   ./scripts/ci/diagnose-and-fix.sh --fix    # also apply auto-fixes
#
# Claude runs this script on CI failure to obtain proposed fixes.
...
check_version_sync() {
  ...
  if [ "$file_version" != "$json_version" ]; then
    ...
    if [ "$AUTO_FIX" = true ]; then
      ...
      echo "  💡 Proposed fix: change the version in plugin.json to \"$file_version\""
    fi
  else
...
check_templates() {
  ...
  if [ ${#missing[@]} -eq 0 ]; then
    ...
    ISSUES_FOUND=$((ISSUES_FOUND + 1))
    echo "  💡 Proposed fix: create the missing files"
  fi
}
...
check_hooks() {
  ...
  if ! jq empty hooks/hooks.json 2>/dev/null; then
    ...
    ISSUES_FOUND=$((ISSUES_FOUND + 1))
    echo "  💡 Proposed fix: check the JSON syntax of hooks/hooks.json"
    return
  fi
  ...
  if [ ${#missing_scripts[@]} -eq 0 ]; then
    ...
    ISSUES_FOUND=$((ISSUES_FOUND + 1))
    echo "  💡 Proposed fix: create the missing scripts, or remove the references from hooks.json"
  fi
}
...
check_version_bump() {
  ...
  if [ "$AUTO_FIX" = true ] && ! bash ./scripts/sync-version.sh check >/dev/null 2>&1; then
    ...
    if bash ./scripts/ci/check-version-bump.sh >/dev/null 2>&1; then
      echo "  ✅ Recovered release metadata consistency by syncing plugin.json"
      return
    fi
  fi
  ...
  echo "     - Only at release time, update VERSION / plugin.json / CHANGELOG release entry together"
}
