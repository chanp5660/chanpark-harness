...
#
# Purpose:
# - Do not require a VERSION bump for normal PRs
# - Only when VERSION is updated, verify that plugin.json / CHANGELOG release entry are in place
#
# Usage:
...
if [ -n "${GITHUB_BASE_REF:-}" ]; then
  ...
  BASE="HEAD~1"
  DIFF_TARGET="HEAD"
  echo "📌 Push mode: comparing against the previous commit"
else
  BASE="origin/main"
  DIFF_TARGET=""
...
if [ "$CURRENT_VERSION" = "$BASE_VERSION" ]; then
  echo "✅ VERSION is unchanged. A version bump is not required for normal PR / normal push."
  ...
  echo "❌ VERSION is unchanged but does not match plugin.json."
  bash ./scripts/sync-version.sh check
  exit 1
fi

if ! semver_gt "$CURRENT_VERSION" "$BASE_VERSION"; then
  echo "❌ VERSION was updated but is not a SemVer increment."
  echo "   Base:    $BASE_VERSION"
  echo "   Current: $CURRENT_VERSION"
  exit 1
fi

echo "✅ Detected a release VERSION update: $BASE_VERSION → $CURRENT_VERSION"
...
if [ "$ERRORS" -gt 0 ]; then
  ...
  echo "  - Only when cutting a release, update VERSION / plugin.json / CHANGELOG release entry together"
  exit 1
fi
