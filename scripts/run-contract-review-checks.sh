...
# Run the runtime_validation defined in sprint-contract.json in order and
# save the results as a review artifact.
...
# Output an absolute path (so the Lead can resolve it even when called from a worktree)
...
# If runtime_validation is empty, signal a downgrade to the static profile.
# Rather than returning APPROVE/REQUEST_CHANGES while empty,
