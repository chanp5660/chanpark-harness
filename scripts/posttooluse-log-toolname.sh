...
#
# Usage: auto-executed from the PostToolUse hook (matcher="*")
# Input: stdin JSON (Claude Code hooks)
# Output:
#   - append JSONL to .claude/state/tool-events.jsonl (only when Phase0 logging is enabled)
#   - update .claude/state/tooling-policy.json (always, when an LSP-related tool is detected)
#
# Control: collect logs only when CC_HARNESS_PHASE0_LOG=1 is set
#       (disable after tool_name is determined to prevent log bloat)
#       LSP tracking always runs (independent of matcher "LSP", to avoid deadlock)
...
# Acquire a lock (prefer flock, otherwise mkdir lock)
acquire_lock() {
  local lockfile="$1"
  local timeout=5
  local waited=0
  ...
  # If flock is unavailable, use mkdir lock (atomic)
...
# Extract needed fields from JSON (prefer jq, otherwise python3)
TOOL_NAME=""
SESSION_ID=""
FILE_PATH=""
COMMAND=""
...
# ===== LSP tracking (always runs, avoiding matcher dependency) =====
...
if [ "${CC_HARNESS_PHASE0_LOG:-0}" = "1" ]; then
  ...
  # Create JSONL entry (minimal fields only)
  JSONL_ENTRY=$(cat <<EOF
...
EOF
  ...
  if ! acquire_lock "$LOCK_FILE"; then
    # Skip if the lock cannot be acquired (failure is harmless)
    exit 0
  fi
  ...
  # Append to log (not atomic, but protected by the lock)
...
append_session_event() {
  ...
  if command -v jq >/dev/null 2>&1; then
    ...
    # Append to event log (SESSION_ORCHESTRATION.md unified schema)
    if [ -n "$data_json" ]; then
