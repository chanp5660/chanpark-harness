#!/bin/bash
# session-list.sh
# Show a list of active sessions
#
# Usage:
#   ./session-list.sh
#
...
main() {
  ...
  if ! command -v jq >/dev/null 2>&1; then
    echo "  ⚠️ jq is not installed, cannot show details"
    exit 0
  fi

  # Display while cleaning up stale sessions
  ...
  echo ""
  echo "💡 Tips:"
  echo "  - /session broadcast \"message\" notifies all sessions"
  echo "  - /session inbox shows received messages"
}
