...
#   --data <json>    : Additional event data (optional)
#
# Output:
#   On success: exit 0
...
# Valid states list (kept in sync with the States in docs/SESSION_ORCHESTRATION.md)
...
get_retry_backoff() {
  ...
  if [ -f "$CONFIG_FILE" ]; then
    ...
    if [ -n "$backoff_line" ]; then
      # Parse from a YAML array [5, 15, 30]
      local arr
...
PY
  ...
  # Create a new session file if none exists
  mkdir -p "$STATE_DIR" 2>/dev/null || true
