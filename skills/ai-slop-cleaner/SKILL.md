...
- Good fit: `chanpark-harness:ai-slop-cleaner skills/foo/SKILL.md skills/bar/SKILL.md`
- Good fit: a planning or implementation session handing off only the files changed in that session
...
This skill can be invoked as a bounded post-review cleanup pass from any implementation session.

- In that workflow, the cleaner runs in standard mode (not `--review`)
- The cleanup scope is the session's changed files only
- After the cleanup pass, the caller re-runs regression verification before completion
- `--review` remains the reviewer-only follow-up mode, not the default integration path
...
- `/chanpark-harness:ai-slop-cleaner <target>`
- `/chanpark-harness:ai-slop-cleaner <target> --review`
- `/chanpark-harness:ai-slop-cleaner <file-a> <file-b> <file-c>`
- From an implementation session: run the cleaner on that session's changed files only, then return to the caller for post-cleanup regression verification
