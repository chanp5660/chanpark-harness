...
Meta-skill for managing chanpark-harness skills via CLI-like commands.
...
2. Scan user skills at `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/learned/`
3. Scan project skills at `.claude/skills/`
4. Parse YAML frontmatter for metadata
5. Display in organized table format:

```
BUILT-IN SKILLS (bundled with chanpark-harness):
| Name              | Description                    | Scope    |
|-------------------|--------------------------------|----------|
| visual-verdict    | Structured visual QA verdicts  | built-in |
| harness-plan      | Research-backed task planning  | built-in |
...
**Built-in skill note:** Built-in skills are bundled with chanpark-harness and are discoverable/readable, but not removed or edited through `/skill remove` or `/skill edit`.
...
5. **Ask for scope:**
   - `user` → `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/learned/<name>/SKILL.md`
...
1. **Search for skill** in both scopes:
   - `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/learned/<name>/SKILL.md`
   - `.claude/skills/<name>/SKILL.md`
2. **If found:**
   - Display skill info (name, description, scope)
   - **Ask for confirmation:** "Delete '<name>' skill from <scope>? (yes/no)"
3. **If confirmed:**
   - Delete entire skill directory (e.g., `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/learned/<name>/`)
...
File Path: /home/user/.claude/skills/learned/custom-logger/SKILL.md
...
**Behavior:**
1. **Scan both scopes:**
   - User skills: `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/learned/`
   - Project skills: `.claude/skills/`
...
USER_SKILLS_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/learned"
...
if [ -d "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/learned" ]; then
  USER_COUNT=$(find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/learned" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/learned" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
Report findings and ask if user wants to extract any as skills (invoke `/skillify`).
...
- `/skill add` - Start skill creation (invoke skillify)
...
**Version Control**: Project-level skills (`.claude/skills/`) are intended to be committed with your code so the whole team benefits.
...
- `/skillify` - Extract a skill from current conversation
