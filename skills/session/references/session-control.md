...
- resume:
  ...
  - Otherwise, if `resume_session_id` is provided → `--resume <id>`
- fork:
  - If `fork_session_id` is provided → `--fork <id>`, otherwise `--fork current`
  - If `fork_reason` is provided → `--reason "<text>"`
