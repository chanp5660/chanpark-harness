---
name: trace
description: Evidence-driven tracing skill that orchestrates competing tracer hypotheses to explain why an observed result happened — generates rival explanations, gathers supporting and contradicting evidence in parallel, runs a rebuttal round, and surfaces the single best discriminating probe to collapse remaining uncertainty.
...
This is the orchestration layer on top of the built-in `tracer` agent. The goal is to make tracing feel like a reusable operating lane within chanpark-harness: restate the observation, generate competing explanations, gather evidence in parallel, rank the explanations, and propose the next probe that would collapse uncertainty fastest.
...
Use **Claude built-in team mode** for tracing.
