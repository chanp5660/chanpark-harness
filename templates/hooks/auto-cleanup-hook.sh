...
# PostToolUse Hook: Automatically checks file size after writing to Plans.md and similar files
#
# Environment variables:
#   $CLAUDE_FILE_PATHS - Space-separated list of changed file paths
#
# Configuration:
...
# Variable to accumulate feedback messages
FEEDBACK=""
  ...
  if [[ "$file" == *"Plans.md"* ]] || [[ "$file" == *"plans.md"* ]] || [[ "$file" == *"PLANS.MD"* ]]; then
    if [ -f "$file" ]; then
      ...
      if [ "$lines" -gt "$PLANS_MAX_LINES" ]; then
        FEEDBACK="${FEEDBACK}Plans.md is ${lines} lines (limit: ${PLANS_MAX_LINES}). It is recommended to archive old tasks with \`/maintenance\`.\n"
      fi
    fi
  fi
  ...
  if [[ "$file" == *"session-log.md"* ]]; then
    if [ -f "$file" ]; then
      ...
      if [ "$lines" -gt "$SESSION_LOG_MAX_LINES" ]; then
        FEEDBACK="${FEEDBACK}session-log.md is ${lines} lines (limit: ${SESSION_LOG_MAX_LINES}). It is recommended to split it by month with \`/maintenance\`.\n"
      fi
    fi
  fi
  ...
  if [[ "$file" == *"CLAUDE.md"* ]] || [[ "$file" == *"claude.md"* ]]; then
    if [ -f "$file" ]; then
      ...
      if [ "$lines" -gt "$CLAUDE_MD_MAX_LINES" ]; then
        FEEDBACK="${FEEDBACK}CLAUDE.md is ${lines} lines. Consider moving information that is not always needed into docs/ and referencing it with \`@docs/filename.md\`.\n"
      fi
    fi
  fi
done
