...
- 작성일: 2026-08-11 · 개정: 2026-08-11 (독립 비평 라운드 2 반영)

> **개정 요약 — 라운드 2에서 실제로 바뀐 결정 4개.** 초판은 팽창 경로 네 개를 열어둔 채 통과
> 시도를 했고, 비평이 전부 재현했다. 바뀐 것만 적는다.
>
> | § | 이전 결정 | 개정 결정 | 왜 |
> |---|-----------|-----------|-----|
> | 2.3 | T2 아카이브 = **파일 전체**가 종결일 때 | T2 = **행 단위**. 종결 행 하나하나가 `archive_days` 경과 시 후보 | 끝나지 않는 `cc:todo` 하나가 파일 전체의 아카이브를 영구 차단했다. 120 done + 1 todo = 125줄인데 스윕은 "no candidates". Linear의 실제 규칙도 issue 단위다 |
> | 2.4 | 스킬이 Jaccard를 "계산한다"(산문) | `scripts/plans-dupe-check.sh` **실행 파일**. 호출 지점은 `add` 스텝 2. 코퍼스는 `Plans.md` **+ 살아있는 백로그 불릿** | 산문은 메커니즘이 아니다. 게다가 비교 대상이 신규 항목이 실제로 쌓이는 파일(백로그)을 빼고 있었다 |
> | 2.5 | 줄 캡 **기각** | 행 캡은 유지 + **`max_lines = 80` 보고형 예산 신설**. 하드 캡 30행에서 93줄이던 보일러플레이트를 48줄로 잘라 도달 가능하게 만듦 | 행 캡은 활성 행만 센다. 종결 행과 보일러플레이트는 무료였고, 하드 캡에서 이미 한 화면을 넘겼다 |
> | 2.2 | 백로그의 출구는 승격 **하나** | 출구 **넷**: promote / decline / duplicate / snooze. + T3 캡처일 기준 노후 리포트 | 기본 목적지가 유일하게 라이프사이클이 없는 파일이었다. 마커가 금지되므로 `cc:dropped`가 도달할 수 없고, 결국 "조용한 삭제 아니면 무한 누적" 둘뿐이었다 |
>
> 공통 원칙은 안 바뀌었다: **전부 보고만 하고 아무것도 자동으로 쓰지 않는다.**
...
| 별도 백로그 파일 vs 백로그 없음? | **별도 파일, 단 마커 없이. 출구는 넷**(promote / decline / duplicate / snooze) + T3 노후 리포트. |
| 자동 만료 며칠? | **14일. 행 단위. 제안만, 절대 자동 기록 안 함.** 유도는 §2.3. |
| 중복 판정 주체? | **`scripts/plans-dupe-check.sh` 실행 파일을 `harness-plan`이 호출.** 자문만, 절대 차단 안 함. |
| 활성 파일 하드 캡? | **태스크 행 25 soft / 30 hard**(초과 시 백로그로 fail-closed) **+ `max_lines = 80` 보고형 길이 예산**. |
...
#### 기본 목적지에는 라이프사이클이 있어야 한다 — 출구 4개 + T3 (개정)

초판은 이 파일을 "모든 신규 항목의 **기본 목적지**"로 지정하면서 동시에 "캡 없음, 스윕 없음,
만료 없음, 중복 검사 없음, 청소 없음"이라고 선언했다. 그 둘을 같이 쓰면 **정의상 무한 증가**다.
"아무도 안 읽으니까 괜찮다"는 방어는 이 문서 스스로 실패로 랭크한 OpenSpec/분산 리스트의
약점과 정확히 같은 문장이다.

더 구체적인 구멍: 백로그 항목은 **마커를 갖지 않는다**(그게 이 파일의 존재 이유다). 따라서
조각 A(`cc:dropped`)가 **백로그에는 도달할 수 없다.** "안 하기로 했다"를 기록할 방법이 없고,
출구는 승격 아니면 조용한 삭제 둘뿐이었다. Linear의 Triage는 네 가지 처분을 준다 —
accept / duplicate / decline / snooze. 초판은 하나만 줬다.

개정: **마커가 금지되므로 처분을 텍스트가 나른다.**

| 처분 | 표기 | 효과 |
|------|------|------|
| **Promote** (accept) | `Plans.md` 표 행으로 다시 쓰고 불릿 삭제 | 활성 집합 진입 |
| **Decline** | `- ~~<원문>~~ declined <YYYY-MM-DD>: <이유>` | 은퇴. **기록은 남는다.** 백로그판 `cc:dropped` |
| **Duplicate** | 살아남는 항목을 이유에 적고 decline | 흔적을 잃지 않고 은퇴 |
| **Snooze** | 불릿을 오늘 날짜로 다시 씀 | 노후 시계 리셋. 명시적이고 눈에 보임 |

취소선 불릿은 **어디서나 불활성**이다: session-monitor의 live 카운트에서 빠지고, T3 리포트에서
건너뛰어지고, `plans-dupe-check.sh` 코퍼스에서도 빠진다(은퇴한 아이디어가 자기 후속을 영원히
신고하면 안 된다). 쌓이면 종결 행과 같은 곳
(`.claude/memory/archive/Plans-<date>.md`)으로 일괄 이동한다.

**불릿을 지워서 은퇴시키는 것만 금지한다.** 삭제는 기록이 남지 않는 유일한 처분이고, 이 넷은
정확히 그걸 대체하려고 있다.

**T3 — 캡처일 기준 노후 리포트.** 처분이 있어도 눈에 안 띄면 아무도 안 한다. `plans-sweep.sh`가
`backlog_stale_days`(기본 90일)를 넘긴 **살아있는** 불릿을 보고한다. T1/T2와 똑같이 **읽기 전용**.

90일 유도(적합이 아니라 경계):

```
Shape Up 베팅 사이클 = 6주
두 사이클을 온전히 살아남았는데 한 번도 피치되지 않은 아이디어
  = 이 백로그가 가진 유일한 선별 과정을 통과 못 한 것        = 84 일
사람이 실제로 검토하는 분기 경계로 올림                      = 90 일
```

T3가 임계값 적합을 필요로 하지 않는다는 점이 T1과의 차이다: 캡처일은 포맷이 이미 요구하는
값이라 **나이 자체는 측정값**이고, 90일은 "그 나이를 언제 출력할 가치가 있는가"만 정한다.

**이 파일에는 여전히 행 캡도 라인 예산도 없다.** 캡처를 캡하면 손실이 기록되지 않는 곳으로
옮겨갈 뿐이다. 대신 나이를 보고하고 출구를 넷 준다.

가드: `plans-backlog-lifecycle`(픽스처 `tests/fixtures/plans-backlog-lifecycle.md`). 네 가지 음성
대조가 전부 FAIL을 만든다(검증 완료) — 설정에서 T3 끄기, 스크립트 기본값에서 T3 끄기,
declined 불릿을 stale로 보고하게 만들기, 파일에서 처분 문서 제거.
...
| 트리거 | 조건 | 산출 | 기본 |
|--------|------|------|------|
| **T1 — stale 제안** (약한 근거) | `cc:todo` 또는 `cc:blocked` **행**이 14일간 무터치 | `cc:dropped` **후보로 보고**. 기록 안 함. | 꺼짐 |
| **T2 — 아카이브 제안** (강한 근거) | 종결(`cc:done`/`cc:dropped`) **행**이 14일간 무터치 | 그 행들의 아카이브 이동 **제안** | 켜짐 |
| **T3 — 백로그 노후 리포트** | `Plans-backlog.md`의 **살아있는 불릿**의 캡처일이 90일 경과 | 4개 처분 중 하나를 하라고 **보고** | 켜짐 |
...
#### T2는 파일 조건이 아니라 **행 조건**이다 (개정)

초판 T2는 "**모든** 행이 종결 **그리고** 파일 14일 무편집"이었다. 이건 팽창 경로를 그대로
열어둔 것이었고, 실측으로 재현됐다:

```
Plans.md = cc:done 120행 + cc:todo 1행 → 125줄
  scripts/plans-sweep.sh  → "no candidates (active 1, terminal 120/121)"
  session-monitor         → 경고 없음
  soft_cap / hard_cap     → 활성 행만 세므로 1/30, 발화 안 함
  T1                      → cc:todo/cc:blocked만 대상 + 기본 꺼짐
```

즉 **끝나지 않는 태스크 하나**가 임의로 큰 종결 행 더미를 활성 파일에 영구히 묶어둔다. 실패 2번
("안 읽힐 때까지 커진다")이 기본으로 열리는 그 파일에서 그대로 살아남는다. 게다가
`skills/maintenance/references/cleanup.md`는 "스윕에서 아카이브하라, 손으로 하지 마라"라고
수동 우회까지 금지하고 있었다 — 출구가 아예 없었다.

**이 문서가 인용한 Linear 규칙 자체가 행(issue) 단위다.** "Closed issues are auto-archived after
they have remained completed, canceled, or auto-closed and inactive for the full auto-archive
period." 뷰 단위도 프로젝트 단위도 아니다. "아카이브는 상태와 시간의 순수 함수"라고 할 때의
그 **상태는 아카이브되는 대상의 상태**여야 한다. 대상이 행이면 조건도 행이다.

개정 규칙:

```
∀ row ∈ Plans.md:
    status(row) ∈ {cc:done, cc:dropped}  ∧  now − blame_author_time(row) ≥ archive_days
    ⇒ row 는 아카이브 후보
```

파일에 무엇이 더 있든 무관하다. 활성 행은 제자리에 남고, 종결 행만 나간다. 캡이 활성 행만
세는 것(§2.5)과도 이제 서로 싸우지 않는다 — 캡은 활성 행을, T2는 종결 행을 본다. 서로소다.

폴백: `git blame`을 못 쓰는 환경(저장소 밖, 추적 안 되는 파일)에서는 파일 mtime 기준의 더 약한
신호로 내려가고, 출력에 그렇게 적는다. 규칙을 조용히 바꾸지 않고 **강도가 떨어졌다고 말한다.**

가드: `check-regression-guard.sh` / `plans-archive-per-row`, 픽스처
`tests/fixtures/plans-terminal-pile.md`(종결 12행 + 활성 1행). 규칙을 파일 단위로 되돌리면
`PLANS_ACTIVE = 1`이라 스윕이 침묵하고 케이스가 **FAIL**한다(검증 완료).
...
#### 실행 파일로 내린다 — 산문은 메커니즘이 아니다 (개정)

초판은 "스킬이 Jaccard를 계산한다"를 문장 하나로 적어두고 끝냈다. 그건 결정이지 구현이 아니고,
비평이 정확히 짚었다: `grep -rn Jaccard skills/` 결과가 산문 한 줄뿐. 스크립트도, 호출 지점도
없었다. 더 나쁜 문제가 하나 더 있었다 — 비교 대상이 `Plans.md`의 행인데, **같은 스킬의
backlog-by-default 규칙이 신규 항목을 `Plans-backlog.md`로 보낸다.** 중복이 실제로 쌓이는
유일한 파일이 유일하게 비교되지 않는 파일이었다.

개정: **`scripts/plans-dupe-check.sh`**.

| 항목 | 값 |
|------|-----|
| 호출 | `bash scripts/plans-dupe-check.sh "<새 설명>"` |
| 호출 지점 | `harness-plan` → `add` → **스텝 2**, 목적지를 고르기 전 (SKILL.md에 순서로 명시) |
| 코퍼스 | `Plans.md`의 모든 태스크 행 Task 셀 **+** `Plans-backlog.md`의 **살아있는** 캡처 불릿 |
| 제외 | 취소선 처리된(declined) 불릿 — 은퇴한 아이디어가 자기 후속을 영원히 신고하면 안 된다 |
| 정규화 | 소문자화, `[tdd:*]` / `[P]` / 코드 스팬 제거, `[a-z0-9]+` 토큰. **불용어 제거 없음** (임계값을 raw word set에서 유도했으므로 토크나이저를 바꾸면 임계값이 무효가 된다) |
| 임계값 | 0.4, `--threshold`로 조정 |
| 종료 코드 | 0 = 후보 없음, **1 = 후보 있음(에러 아님)** |
| 성질 | 출력하고 진행. **쓰기를 거부하지 않는다** |

`harness-plan`은 이미 append하려고 파일을 읽으므로 추가 I/O 부담이 사실상 없다.

가드: `plans-dupe-check`. 네 가지 음성 대조가 전부 FAIL을 만든다(검증 완료) — 스크립트 삭제,
코퍼스를 `Plans.md`로 축소, declined 불릿을 코퍼스에 재편입, 스킬에서 호출 지점 제거.
...
#### 행 캡은 파일을 못 묶는다 — `max_lines = 80` 신설 (개정)

초판은 라인 캡을 아래 표에서 **기각**했다. 그 기각은 절반만 옳았고, 나머지 절반이 실패 2번을
그대로 통과시켰다. 실측:

```
활성 행 = 30 (하드 캡 정확히) → 구 레이아웃 Plans.md = 93 줄
  → 이미 한 화면 밖. 어떤 표면도 아무 말도 하지 않음.
  → soft/hard 캡은 정상. WIP 캡도 정상. 아무것도 발화하지 않음.
```

행 캡이 파일을 못 묶는 이유는 구조적이다:

1. **활성 행만 센다**(§2.5 상단, "아직 아카이브 안 된 종결 행은 캡에 안 들어간다"). 종결 행은
   길이를 무제한으로 늘리면서 카운트를 안 건드린다.
2. **보일러플레이트가 공짜다.** 구 `Plans.md`의 범례+포맷 설명만 40줄이 넘었다. 행이 0개여도
   63줄이었다.

**기각 이유는 유지하되, 적용 대상을 바꾼다.** 원래 기각 근거("설명 길이에 좌우되어 좋은 서술을
벌준다")는 **입장 통제(admission control)로서의** 라인 캡에 대해서만 옳다. 잘 쓴 설명 때문에
쓰기를 거부당하면 사람은 설명을 나쁘게 쓴다. 하지만 **보고**는 아무도 벌하지 않는다. 그래서:

| 예산 | 값 | 세는 대상 | 오버플로 동작 |
|------|-----|-----------|----------------|
| soft 행 캡 | 25 | 활성 행 | 경고 + 신규 항목을 백로그로 라우팅 |
| hard 행 캡 | 30 | 활성 행 | **`Plans.md` 추가 거부.** 백로그로 라우팅 |
| WIP 캡 | 1 | `cc:wip` 행 | 하나만 |
| **`max_lines`** | **80** | **파일 전체(보일러플레이트 포함)** | **보고하고 진행. 절대 거부 안 함.** 초과분을 숫자로 말하고 아카이브 스윕을 지시. 이 저장소에서는 CI(`plans-line-budget`)가 추가로 실패 |

행 캡과 길이 예산은 **오버플로 방향이 반대이고, 그건 의도다.** 행 초과는 거부(회복 가능한
백로그로 밀어냄), 길이 초과는 보고(고칠 방법이 항상 아카이브지, 덜 쓰기가 아님).

**80의 근거.** 이건 적합값이 아니라 관례다: 이 저장소의 모든 스타일 규칙이 이미 80을 쓰고 있어
독자가 이미 감을 갖고 있는 숫자다. 정직한 주장은 "80이 최적"이 아니라 "한 화면 관례에 맞고,
하드 캡에서 도달 가능하다"이다.

**둘을 화해시키려면 뭔가 줄어야 한다. 보일러플레이트를 줄였다** (행 캡이 아니라):

```
장문 범례 + 포맷 규칙 → skills/harness-plan/references/plans-format.md 로 이동
템플릿 보일러플레이트   48 줄
+ 하드 캡                30 행
= 78 줄  ≤ 80            ✓ 도달 가능
이 저장소 실제 Plans.md  44 줄 (0행) → 30행 시 74 줄
```

행 캡을 줄이지 않은 이유: 30은 관측된 완주 최대 계획(20행)에서 유도됐고, 예산을 맞추려고
관측 근거가 있는 숫자를 깎는 건 근거가 없는 쪽을 근거 있는 쪽에 우선시키는 것이다.
보일러플레이트에는 그런 근거가 애초에 없다.

**어디서 보고되나 (§2.5 → 헤드리스 구멍 메우기).** 초판의 유일한 기계적 표면은 session-monitor
였는데, `CLAUDE.md`가 스스로 적어놓았듯 **헤드리스 `-p` 모드에서는 그 블록이 아예 안 뜬다.**
헤드리스 에이전트가 50행을 붙여도 어디서도 한 마디가 안 나온다. 그래서 두 곳에서 보고한다:

| 표면 | 언제 | 헤드리스 |
|------|------|----------|
| `scripts/hook-handlers/session-monitor.sh` | 세션 시작 | ✗ |
| `scripts/hook-handlers/plans-watcher.sh` (PostToolUse) | `Plans.md`에 쓸 때마다 | **✓** |
| `scripts/ci/check-regression-guard.sh` (`plans-line-budget`) | CI | ✓ |

plans-watcher는 PostToolUse라 **구조적으로 차단이 불가능하다** — 쓰기가 이미 끝난 뒤에 돌고
페이로드가 `additionalContext`뿐이다. 알려줄 수만 있고 거부할 수는 없다. 이 저장소의
wip-guard 사고가 fail-closed plans 훅의 비용을 이미 보여줬으므로, 그게 맞는 거래다.

가드 `plans-line-budget`이 네 가지를 동시에 고정한다: ① `max_lines`가 존재하고 ≤ 80,
② 이 저장소의 `Plans.md`가 그 안에 있고, ③ **템플릿 보일러플레이트 + 하드 캡이 예산 안에
들어와서 예산이 실제로 도달 가능하며**, ④ 두 훅 모두가 실제로 보고한다. 다섯 가지 음성 대조가
전부 FAIL을 만든다(검증 완료).
...
| 바이트/라인 캡을 **입장 통제로** 사용 | 설명 길이에 좌우된다. 잘 문서화된 태스크 10개가 한 줄짜리 40개보다 먼저 걸린다 — 좋은 서술을 벌준다. **보고형으로는 채택**(위 표) — 보고는 아무도 벌하지 않고, 행 캡이 구조적으로 못 보는 경우를 잡는다. |
| 라인 예산을 맞추려고 하드 캡을 30 → 20으로 축소 | 30은 관측 데이터(완주 최대 20행 × 1.5)에서 유도됐고 보일러플레이트는 아니다. 근거 있는 숫자를 근거 없는 숫자에 맞춰 깎는 건 방향이 반대다. |
| 라인 예산을 환경변수로 노출 | 호출마다 올릴 수 있는 예산은 예산이 아니다. `harness.toml`에서만 바꾼다. |
...
활성 행 ≤ 25(soft) / 30(hard). `cc:wip` ≤ 1. **파일 전체 ≤ 80줄**(보고형, §2.5). 장문 범례와
포맷 규칙은 `skills/harness-plan/references/plans-format.md`에 있다 — 예산을 맞추려고 내보냈다.

**`Plans-backlog.md`** — 평문 불릿, 캡처 날짜 포함, **`cc:` 마커 없음**. 표 행도 아니다
(표준 카운터가 표 행만 세므로 이중 안전). **행 캡도 라인 예산도 없다** — 캡처를 캡하면 손실이
기록 안 되는 곳으로 옮겨갈 뿐이다. 대신 **출구가 넷**(promote / decline / duplicate / snooze)이고
T3가 90일 넘은 살아있는 불릿을 보고한다(§2.2). 취소선 불릿 = declined = 이 파일의 종결 상태.

**`.claude/memory/archive/Plans-<date>.md`** — 종결 **행**이 이동해 온다(파일 단위가 아니라 행
단위 — §2.3). 쌓인 declined 불릿도 여기로 일괄 이동할 수 있다. **어떤 카운터도 절대 읽지
...
| `max_lines = 80`이 관례이지 적합값이 아님 | §2.5에 그렇게 적음. 설정 가능. 초과해도 **거부가 아니라 보고**라 잘못 잡아도 흐름을 안 막는다. |
| `backlog_stale_days = 90`도 적합값이 아님 | 같은 처리. 캡처일 자체는 측정값이고, 90은 "언제 출력할지"만 정한다. 읽기 전용. |
| 캡·라우팅·WIP≤1 등 **나머지 규칙은 여전히 LLM 산문**이고 아무것도 쓰기를 거부하지 않음 | 의도적 fail-open(§2.4의 hook 기각 논거와 동일). 다만 침묵은 고쳤다: `plans-watcher`(PostToolUse, **헤드리스에서도 발화**)가 하드 캡·라인 예산 초과를 매 쓰기마다 출력하고, CI 가드가 이 저장소에서 이를 고정한다. 남는 갭은 "권고를 무시하면 여전히 쓸 수 있다"이며, 이는 거부형으로 바꿀 때의 비용(세션 락)보다 싸다고 판단한다. |
| 승격·decline이 전부 수동 재작성 | 마찰이 기능(§2.2). 대신 나이 리포트(T3)와 중복 리포트(`plans-dupe-check.sh`)가 "언제 손을 대야 하는지"를 알려준다. |
